...
aliases:
---
After spending a few years in the realm of split ergonomic keyboards, I had mostly stopped paying attention to regular mechanical keyboards. Times sure have changed though; what used to require group buys, long waits, and a suspicious amount of disposable income is now showing up as off-the-shelf boards with gasket mounts, wireless modes, easy programmability, and stock switches that don't need to be meticulously disassembled and lubricated for a whole afternoon.

My attention for these "traditional" input devices was piqued again when Epomaker reached out and asked if I wanted to try a few of their boards. Here were the three boards that caught my eye:
...
This post covers an in-depth review of these boards, where each keyboard section answers the following questions:
...
I’ve been wanting to try a true ortholinear board for years, something about the uniformity and compactness is quite alluring, even if it's not the most ergonomic. So this was a convenient opportunity to finally try one! 
...
For me, the aluminum case is the main selling point. It gives the Luma40 enough mass that it doesn’t slide around on my desk. The rubber pads on the bottom are thin, but they kept the board planted on my desk.
...
The board runs off a seemingly unknown microcontroller, where the marking on it just says `MILLER N25NOV17`. This appears to be a custom, proprietary microcontroller, which manufacturers do because it’s cheaper to produce than using an off-the-shelf development chip like RP2040 or nRF52840, which is commonly used in the custom keyboard community.

![Close-up of the PCB.](DSCF3435.jpg)

The switches are advertised as hot-swappable, but the plate tolerances seemed way too tight on my unit that I stopped after removing one corner switch (which was already a struggle). I had to wiggle it out carefully, and the others felt like they were closer to "technically removable" than "casually swappable." Hopefully just a tolerance issue for this board or batch?

Under the switch there is a small gasket layer between the plate and PCB that the switch sits on top of, for some additional damping.
...
I'm not a huge fan of the default secondary layer, as I prefer to have my numbers arranged in a numpad, and symbols on the other side, as described in my blog post [here](/posts/2025/07/optimizing-symbols-layer/).  But this is easy to change!
...
Don't get me wrong, it's definitely possible to use this as a daily driver, since many people use [Corne keyboards](/posts/2025/05/corne-keyboard/) which has 42 keys, or many even going down to [36 keys](/posts/2025/07/36-key-layout/) or fewer. The main difference with ortholinear layouts is the lack of a true thumb cluster, but there are keys along the bottom row that can be used in place of them.

After a week, I found liked it more than I did on day one. The keycaps still would be the first thing I changed, but the size and case made it easy and fun to type on, along with taking up very little space on my desk (or wherever you may be using this).

# Hack70

## What is it?

At first glance, the Hack70 was the most intriguing board of the three because there aren't many full-sized ortholinear boards, especially with split space bars. You get more keys than the Luma40, less width than many 65% boards, and a split space bar.

The split space bar design is intriguing because it brings the essence of having a thumb cluster to traditional keyboards, where a popular use of it would be turning it into an easy-access layer key. The length of space bars was due to physical and mechanical limitations with typewriters, so it's nice to see a modern take on the design that's a bit more function over form.
...
The backside has a storage slot for the 2.4GHz dongle, and the case includes two levels of flip-out feet. I usually avoid tall rear feet, but they are there if you prefer a steeper typing angle.
...
The split spacebar should have been the feature I liked most, since using thumb clusters is one of the main advantages of modern ergonomic keyboards. One thumb key for space and another for a navigation layer sounded perfect on paper...

The problem? The two spacebars don't line up with the rest of the keys!

While I was using this keyboard, I kept missing space for the first few days and assumed I was just adjusting to a new board. Then I looked down and realized my thumbs were landing on the column gaps instead of the keys.
...
This made using the Hack70 mildly frustrating... Typing felt great, sounded great, but the placement of the most commonly used key was a bit of a letdown.
...
- Either:
	- Option 1:
		- Use 1U for both backspace and enter
		- Drop the backtick/tilde key from its current place, and drop (and shift inwards) the entire outer column of the keyboard
		- Caveat is this would also require losing either shift or page up beside the arrow key
	- Option 2:
		- Use 1U for just the backspace
		- Move the backtick/tilde key to beside backspace

Relatively small changes, but not needing to relearn the placement of the enter key is a real motivator and make the learning curve easier.
...
If you already use ortholinear keyboards, the space bar placement is the thing to check before buying. As I mentioned before, I would have preferred two 2U spacebars under the V/B and N/M columns.

For me, the Hack70 has excellent keyboard components, but sadly is attached to a layout that will take a while for me to get used to. I do appreciate the design language of this board though, and am looking forward to seeing more boards like this. 

# Split70

## What is it?

The Split70 is a row-staggered split keyboard. Unlike many [ergonomic boards](posts/2026/02/beginners-guide-split-keyboards/), it doesn't ask you to relearn column stagger, thumb clusters, or a [36-key layout](/posts/2025/07/36-key-layout/). It keeps the familiar row-staggered key positions and lets you separate the halves.

My unit is white with blue accents, 46gf Wisteria Linear V2 switches, a knob on the left half, and a column of dedicated macro keys.
...
The keycaps are the weakest visible part. The legends are softer than the Hack70's legends, especially on close inspection. They work fine, but this is where the board looks more budget-conscious. However, it also comes in a black colourway and those keycaps look a bit better.

![Close up of the keycaps.](DSCF3301.jpg)

![Split70 in black.](split70-black.png "Epomaker|https://epomaker.com/products/epomaker-split70")
## How does it type?

The Wisteria Linear V2 switches are quiet and lighter than the Hack70's Creamy Jades. Above average, but not as nice feeling or sounding as the Hack70 though.
...
The tradeoff is desk space. A row-staggered split takes more room than the product photos suggest because you need space between the halves for the ergonomic benefit. The inner boarder is also quite large, adding to the overall footprint required.

## What's interesting?

Not much, but that's a good thing for someone looking for a split traditional keyboard?

The duplicated B key on both sides is a nice addition since it alleviates the need to retrain the "B muscle memory". On a normal keyboard, many people hit B with their right index finger even though it technically lives on the left side, so switching to a split usually requires relearning this behaviour.
...
There's quite a few options in this space of split, row-staggered keyboards, so I'd only recommend this one if you like the aesthetics as well as the volume knob and mini macro pad. For me, it takes up too much space on my desk, but this may not be a problem for you.

# So, which one would I keep using?

The Hack70 has the most cushioned switch and case feel of the three. It also has the layout decision that bothered me most, but I think most people can overlook this. It checks the most boxes and is also the cheapest of the these, so it'd be the one I would go for if I had to choose.

The Luma40 is the most limited, but was still enjoyable to use. It is a tiny ortholinear board with a rigid case, odd keycaps, and enough charm that I kept reaching for it even when it made me work harder.
...
I came into this expecting the Hack70 to be the easy favourite. After using all three, I still think it is the most technically interesting board, but the Luma40 is the one I kept thinking about. I still prefer columnar stagger keyboards over any of these, but it's nice to see more options available for those who don't want to [handwire their own custom keyboards](/posts/2025/08/handwired-skeletyl/).
